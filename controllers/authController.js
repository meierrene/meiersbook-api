...
exports.login = catcher(async (req, res, next) => {
  ...
  if (!email || !password)
    return next(
      new ErrorThrower('Please provide a valid email and password', 400)
    );
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    return next(new ErrorThrower('Incorrect email and/or password', 401));
  }
...
exports.protect = catcher(async (req, res, next) => {
  ...
  // To ensure that null is not a string value
...
exports.forgotPassword = catcher(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorThrower('There is no user with this email address', 404)
    );
  }
...
exports.resetPassword = catcher(async (req, res, next) => {
  ...
  if (!user)
    return next(new ErrorThrower('Token is invalid or has expired.', 400));
...
exports.updatePassword = catcher(async (req, res, next) => {
  // 1) Get user from collection
  const user = await User.findById(req.user.id).select('+password');
  if (!user) {
    return next(new ErrorThrower('User has not loaded', 404));
  }

  // 2) Check if POSTed current password is correct
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new ErrorThrower('Your current password is wrong.', 401));
  }
